...
def test_zero_shot_relevance_filter(min_score):
    ...
    y_pred = [
        zero_shot_it_relevance_filter(NewsItem(**item), min_score=min_score)[0] == 1
    ...
    false_positives = [
        item for item, t, p in zip(test_cases, y_true, y_pred) if not t and p
    ]
    false_negatives = [
        item for item, t, p in zip(test_cases, y_true, y_pred) if t and not p
    ]
...
def test_metrics():
    y_true = [item["relevant"] for item in test_cases]
    y_pred = [
        zero_shot_it_relevance_filter(NewsItem(**item), min_score=0.08)[0] == 1
        for item in test_cases
    ]
