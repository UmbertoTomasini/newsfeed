...
from newsfeed.main import accepted_item_ids  # Import global state
from newsfeed.main import accepted_items, app
...
def test_ingest_news():
    ...
    response = client.post(
        "/ingest",
        json=jsonable_encoder([item.model_dump(mode="json") for item in test_items]),
    )
    assert response.status_code == 200
...
def test_retrieve_news_sorted_by_recency():
    ...
    assert response.status_code == 200
    retrieved_items = [
        NewsItem(**item_data) for item_data in response.json()
...
def test_deduplication():
    ...
    response = client.post(
        "/ingest", json=jsonable_encoder([duplicate_item.model_dump(mode="json")])
    )
    assert response.status_code == 200
    assert response.json() == {"status": "ACK"}
...
def test_mock_source_ingestion_via_manager():
    ...
    initial_test_mock_items_for_state = [
        NewsItem(
            id="synth-1",
            source="mock-api",
            title="Synthetic Event 1: Critical Security Patch Released",
            body="A major security vulnerability has been discovered and patched in enterprise software.",
            published_at=datetime(2024, 7, 15, 10, 0, 0, tzinfo=timezone.utc),
        ),
        NewsItem(
            id="synth-2",
            source="mock-api",
            title="Synthetic Event 2: New AI Chip Achieves Breakthrough Performance",
            body="Researchers announce a new chip design with unprecedented processing power for AI tasks.",
            published_at=datetime(2024, 7, 15, 11, 30, 0, tzinfo=timezone.utc),
        ),
        NewsItem(
            id="synth-3",
            source="mock-api",
            title="Synthetic Event 3: Cloud Provider Outage Impacts Multiple Services",
        ...
        NewsItem(
            id="synth-4",
            source="mock-api",
            title="Synthetic Event 4: Minor Software Update for Legacy System",
            body="A small patch was released for an older internal system.",
    ...
    ingestion_manager.last_fetched_timestamps["mock-api"] = datetime(
    ...
    mock_items_initial = [
        item for item in retrieved_items_initial if item.source == "mock-api"
    ]
    ...
    mock_items_initial_sorted = sorted(
        mock_items_initial, key=lambda x: x.published_at, reverse=True
    )
    ...
    print(
        f"Before continuous fetch - last_fetched_timestamps: {ingestion_manager.last_fetched_timestamps}"
    )
    ...
    print(
        f"After continuous fetch - new_mock_items_from_continuous_fetch: {[item.published_at for item in new_mock_items_from_continuous_fetch]}"
    )
    ...
    assert len(accepted_items) == (
        len(mock_items_initial) + 2
    )  # Total items increased by 2

    # Verify that the new items are present in the overall accepted_items
    response = client.get("/retrieve")
    assert response.status_code == 200
    retrieved_items_after_continuous = [
        NewsItem(**item_data) for item_data in response.json()
    ]

    ids = [item.id for item in retrieved_items_after_continuous]
    assert "synth-5" in ids
    assert "synth-6" in ids
...
def test_retrieve_updates_with_new_items():
    ...
    retrieved_before_new = [
        NewsItem(**item_data) for item_data in response_before_new.json()
    ]
    ...
    retrieved_after_new = [
        NewsItem(**item_data) for item_data in response_after_new.json()
    ]

    ids = [item.id for item in retrieved_after_new]
    assert "synth-5" in ids
    assert "synth-6" in ids
