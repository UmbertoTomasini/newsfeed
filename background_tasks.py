...
class BackgroundTaskManager:
    ...
    def __init__(self, ingestion_manager, accepted_items: List[NewsItem], 
                 accepted_item_ids: Set[str], all_items: List[NewsItem] = None):
        ...
        while not self.shutdown_flag:
            ...
            if ASSESS_CORRECTNESS_WITH_BIGGER_MODEL:
                result = await loop.run_in_executor(
                    None, 
                    lambda: self.ingestion_manager.continuous_fetch_sources(store_filtered=True)
                )
                ...
                new_items_from_sources = await loop.run_in_executor(
                    None,
                    self.ingestion_manager.continuous_fetch_sources
                )
            ...
            if ASSESS_EFFICIENCY:
                ...
                throughput = newly_added_count / ingestion_latency if ingestion_latency > 0 else 0.0
                
                # Log metrics on separate lines
                log_efficiency(f"Latency: {ingestion_latency:.4f} seconds", step="Ingestion")
                log_efficiency(f"Throughput: {throughput:.2f} items/s", step="Ingestion")
                log_efficiency(f"Items processed: {newly_added_count} new items", step="Ingestion")
                log_resource_usage("Ingestion")
            
            print(f"Finished continuous background ingestion cycle. Added {newly_added_count} new items. "
    ...
    async def initial_fetch_async(self):
        ...
        if ASSESS_CORRECTNESS_WITH_BIGGER_MODEL:
            result = await loop.run_in_executor(
                None,
                lambda: self.ingestion_manager.initial_fetch_sources(store_filtered=True)
            )
            ...
            initial_items = await loop.run_in_executor(
                None,
                self.ingestion_manager.initial_fetch_sources
            )
            filtered_items = []
        ...
        if ASSESS_EFFICIENCY:
            ...
            throughput = initial_added_count / startup_latency if startup_latency > 0 else 0.0
            ...
            log_efficiency(f"Items processed: {initial_added_count} items", step="Startup")
            log_resource_usage("Startup")
        
        print(f"Initial fetch completed. Added {initial_added_count} items. "
              f"Total accumulated items: {len(self.accepted_items)}.")
