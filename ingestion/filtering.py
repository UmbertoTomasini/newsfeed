...
import logging
import re
from newsfeed.config import MIN_SCORE, ASSESS_CORRECTNESS_WITH_BIGGER_MODEL
from sklearn.metrics import precision_score, recall_score, confusion_matrix
import pdb
import warnings
from transformers.utils import logging as hf_logging
from newsfeed.log_utils import log_refused, log_accepted, log_info
import time
...
def get_zero_shot_classifier():
    ...
    if _zero_shot_classifier is None:
        ...
        log_info("Initializing zero-shot classifier (first-time load, this may take 10-30 seconds)...", source="Filtering")
        try:
            ...
            log_info(f"Zero-shot classifier loaded successfully in {time.time() - start_time:.1f} seconds", source="Filtering")
...
ZERO_SHOT_LABELS = [label + RELEVANT_LABEL_SUFFIX for label in RELEVANT_LABELS] + list(NOT_RELEVANT_LABELS)
...
def zero_shot_it_relevance_filter(news_item, min_score=MIN_SCORE, fallback_to_body=True):
    ...
    if not zero_shot_classifier:
        ...
        FILTER_LOGS.append({"filter": "zero_shot", "id": news_item.id, "decision": "pass", "log": log_info})
    ...
    relevant_scores_title = [
        (label, score) for label, score in zip(result_title['labels'], result_title['scores'])
    ...
    top_relevant_title = max(relevant_scores_title, key=lambda x: x[1]) if relevant_scores_title else (None, 0.0)
...
def log_filtering_summary(stage_name, items_with_logs, threshold):
    ...
    logger.info(f"[SUMMARY] {stage_name}: {len(passed)} passed, {len(filtered)} filtered out.")
    FILTER_LOGS.append({"stage": stage_name, "passed": len(passed), "filtered": len(filtered)})
    ...
    for item in news_items:
        ...
        try:
            ...
            if "Now, classify the following article" in relevant_output:
                relevant_output = relevant_output.split("Now, classify the following article", 1)[-1]
            ...
            if re.search(r"\bNOT_RELEVANT\b", output_upper):
                ...
                logger.warning(f"Unexpected label from the larger model: {output} for item {item.id}")
                label = "NOT_RELEVANT"
