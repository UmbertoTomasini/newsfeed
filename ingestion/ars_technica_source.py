...
class ArsTechnicaSource(BaseSource):
    ...
    def fetch_news(self, posts_limit: Optional[int] = None, since_timestamp: Optional[datetime] = None) -> List[NewsItem]:
        ...
        try:
            ...
            if feed.bozo:
                logger.warning(f"Feedparser encountered errors for {self.source_name}: {feed.bozo_exception}")
            ...
            for entry in feed.entries: # Iterate all entries to find recent ones
                ...
                if posts_limit is not None and len(items) >= posts_limit and not since_timestamp:
                    break # Stop if we've collected enough for initial limit

            logger.info(f"Successfully processed {fetched_count} items from {self.source_name} ({len(items)} passing filter/limit).")
            return items

        except Exception as e:
            logger.error(f"Error fetching from Ars Technica source {self.source_name} ({self.feed_url}): {e}", exc_info=True)
            return []