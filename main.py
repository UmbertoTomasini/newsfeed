...
def log_startup_time(message):
    print(f"[STARTUP {time.time() - _startup_time:.2f}s] {message}")

log_startup_time("Starting imports")

import logging
log_startup_time("Imported logging")

from fastapi import FastAPI, HTTPException
from fastapi.responses import HTMLResponse
log_startup_time("Imported FastAPI")

from typing import List, Dict, Set
from newsfeed.models import NewsItem
log_startup_time("Imported models")
...
log_startup_time("Imported sources")

from newsfeed.ingestion.manager import IngestionManager
log_startup_time("Imported manager")

import threading
from collections import defaultdict
from datetime import datetime, timezone
import math
log_startup_time("Imported standard libraries")

from newsfeed.config import MIN_SCORE, MAX_ITEMS, INTERVAL, NUMBER_INITIAL_POST_PER_SOURCE, PERSISTENCE_TIME, ASSESS_CORRECTNESS_WITH_BIGGER_MODEL, ASSESS_EFFICIENCY
log_startup_time("Imported config")

if ASSESS_CORRECTNESS_WITH_BIGGER_MODEL:
    from newsfeed.ingestion.filtering import assess_with_bigger_model, evaluate_pipeline_vs_model
    log_startup_time("Imported filtering (THIS IS LIKELY SLOW)")

import sys
import os
from newsfeed.log_utils import log_accepted, log_refused, log_efficiency, log_resource_usage, log_info, log_error, setup_run_logger
log_startup_time("Imported log_utils")

from newsfeed.background_tasks import BackgroundTaskManager
log_startup_time("Imported background_tasks")

log_startup_time("All imports complete, starting initialization")
...
efficiency_log_file_path = os.path.join(efficiency_log_dir, f'newsfeed_efficiency_{log_time}.log')
...
# Patch log_utils to use the correct loggers
...
ingestion_manager = IngestionManager(
    sources=[reddit_source_sysadmin, reddit_source_outages, reddit_source_cybersecurity, ars_source],
...
def ingest_news(items: List[NewsItem]):
    ...
    if ASSESS_EFFICIENCY:
        ...
        throughput = newly_ingested_count / ingest_latency if ingest_latency > 0 else 0.0
        ...
        log_efficiency(f"Items processed: {newly_ingested_count} new items", step="/ingest")
        log_resource_usage("/ingest")
    
    print(f"Ingest endpoint processed. Added {newly_ingested_count} new items. Total accumulated items: {len(accepted_items)}.")
    return {"status": "ACK"}
...
def retrieve_news():
    ...
    if ASSESS_EFFICIENCY:
        ...
        throughput = len(accepted_items) / retrieve_latency if retrieve_latency > 0 else 0.0
    ...
    return sorted(
        accepted_items,
        key=lambda x: (x.final_score if x.final_score is not None else 0, x.published_at),
        reverse=True
    )
...
def retrieve_all_news():
    ...
    sorted_items = sorted(all_items, key=lambda item: item.final_score if item.final_score is not None else 0, reverse=True)
    if ASSESS_EFFICIENCY:
        retrieve_all_end = time.perf_counter()
        retrieve_all_latency = retrieve_all_end - retrieve_all_start
        throughput = len(sorted_items) / retrieve_all_latency if retrieve_all_latency > 0 else 0.0
        
        # Log metrics on separate lines
        log_efficiency(f"Latency: {retrieve_all_latency:.4f} seconds", step="/retrieve-all")
        log_efficiency(f"Throughput: {throughput:.2f} items/s", step="/retrieve-all")
        log_efficiency(f"Items returned: {len(sorted_items)} items", step="/retrieve-all")
